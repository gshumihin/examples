...
* _URL_ - здесь вы указываете домен (домены), на которых будет работать праило. Работа с домена происходит по аналогии с [правилами сокрытия элементов](#Правила-сокрытия-элементов). Если не указать домен, то правило будет применяться для всех доменов.
